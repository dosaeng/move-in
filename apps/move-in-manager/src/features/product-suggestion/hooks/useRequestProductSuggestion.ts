...
const requestProductSuggestionEndpoint = (filterId: string | number) =>
  `/agent-api/filter-card/${filterId}/recommendation`;

const useRequestProductSuggestion = (
  options?: Omit<
    UseMutationOptions<void, unknown, ProductSuggestionRequestModel, unknown>,
    'mutationFn'
  >
) => {
  return useMutation(async (data: ProductSuggestionRequestModel) => {
    await httpClient.post<ProductSuggestionRequestDTO>(
      requestProductSuggestionEndpoint(data.filterId!),
      {
        body: {
          ...
          filter1_comment:
            data.productSuggestion?.familyPreference?.comment ?? '',
          filter1_qualified_issue:
            data.productSuggestion?.familyPreference?.selected?.map(
              (item) => item.value
            ) ?? [],
          filter2_score: `${
          ...
          filter2_comment:
            data.productSuggestion?.moveInPreference?.comment ?? '',
          filter2_qualified_issue:
            data.productSuggestion?.moveInPreference?.selected?.map(
              (item) => item.value
            ) ?? [],
          filter3_score: `${
            data.productSuggestion?.costPreference?.score ?? 0
          }`,
          filter3_comment:
            data.productSuggestion?.costPreference?.comment ?? '',
          filter3_qualified_issue:
            data.productSuggestion?.costPreference?.selected?.map(
              (item) => item.value
            ) ?? [],
          filter4_score: `${
          ...
          filter4_comment:
            data.productSuggestion?.productPreference?.comment ?? '',
          filter4_qualified_issue:
            data.productSuggestion?.productPreference?.selected?.map(
              (item) => item.value
            ) ?? [],
          filter5_score: `${
          ...
          filter5_comment:
            data.productSuggestion?.lifestylePreference?.comment ?? '',
          filter5_qualified_issue: [
            data.productSuggestion?.lifestylePreference?.traffic?.map(
              (item) => item.value
            ),
            data.productSuggestion?.lifestylePreference?.livingOption?.map(
              (item) => item.value
            ),
            data.productSuggestion?.lifestylePreference?.communityLife?.map(
              (item) => item.value
            ),
            data.productSuggestion?.lifestylePreference?.livingInfra?.map(
              (item) => item.value
            ),
            data.productSuggestion?.lifestylePreference?.educationLife?.map(
              (item) => item.value
            ),
            data.productSuggestion?.lifestylePreference?.deliveryLife?.map(
              (item) => item.value
            ),
          ]
...
defineMock((mock) => {
  mock.post(
    new RegExp(`^${requestProductSuggestionEndpoint('[0-9]+')}$`),
    async (request) => {
