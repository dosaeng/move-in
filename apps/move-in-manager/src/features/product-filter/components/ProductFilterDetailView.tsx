...
import { ChipButtonList, Divider } from '@move-in/design-system';
...
const ProductFilterDetailView: React.FC<Props> = ({ className, filterId }) => {
  ...
  const { data, isLoading } = useProductFilterDetail(filterId);
      ...
      >
        ...
        <ProductFilterSubView title="원하는 집의 조건">
          <ProductFilterSubTagView title="위치">
            <ChipButtonList
              options={data?.productPreference.position ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="매물 구성">
            <ChipButtonList
              options={data?.productPreference.type ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
        </ProductFilterSubView>
        <ProductFilterSubView title="라이프 스타일">
          <ProductFilterSubTagView title="교통권">
            <ChipButtonList
              options={data?.lifestylePreference.traffic ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="희망 구성 옵션">
            <ChipButtonList
              options={data?.lifestylePreference.livingOption ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="공동 생활">
            <ChipButtonList
              options={data?.lifestylePreference.communityLife ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="생활권">
            <ChipButtonList
              options={data?.lifestylePreference.livingInfra ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="학군">
            <ChipButtonList
              options={data?.lifestylePreference.educationLife ?? []}
              readOnly
            />
          </ProductFilterSubTagView>
          <ProductFilterSubTagView title="배달권">
            <ChipButtonList
              options={data?.lifestylePreference.deliveryLife}
              readOnly
            />
          </ProductFilterSubTagView>
        </ProductFilterSubView>
...
const ProductFilterSubView: React.FC<
...
const ProductFilterSubTagView: React.FC<
  React.PropsWithChildren<{ title: string }>
> = ({ title, children }) => {
  return (
    <div
    ...
    >
      <div className={css({ textStyle: 'body-12-r', color: 'text.dark.01' })}>
        {title}
      </div>
      {children}
    </div>
